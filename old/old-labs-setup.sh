...
# Info         : This script is still in development. If you found any issues, please open a issue in the github repo.
...
main_menu () {
    $ECHO "${ORANGE}\n\tSelfmade Ninja Labs Setup v1.12${NC} 🐧\n"
    $ECHO "${BLUE}\t   $LABS\n${NC}"
    $ECHO "${BLUE}Only Supported on unix/linux systems only\n${NC}"
    $ECHO "  Created by ${BLUE}https://git.selfmade.ninja/Henry${NC}\n"
    ...
    if [ -f "$OS_RELEASE" ]; then
        ...
        $ECHO "${BLUE}OS: ${NC}Unknown OS ${RED}\t[Not-Detected] [Not supported]${NC}"
    fi
    $ECHO "${RED}Root Privileges ${NC} ${GREEN}[Detected]${NC}\n"
    
    $ECHO "${ORANGE}[!] NOTE:${NC}"
    $ECHO "${ORANGE}\tThis script is still in development. If you found any issues, \n\tplease open a issue in the github repo.\n${NC}"
...
wg_gen_key () {
    ...
    if [[ -f $(pwd)/privatekey && $(pwd)/pubkey ]]; then
        ...
        $ECHO "${BLUE}[!] Wireguard keys${NC}"
        $ECHO "${GREEN}[+] PublicKey: $(cat publickey)${NC}"
        $ECHO "${GREEN}[+] PrivateKey: ${RED}(hidden)${NC}${NC}"
        $ECHO "${GREEN}[*] Copy and paste the publickey to ${LABS}/devices/add${NC}"
    else
